...
/**
 * Destroy a configuration section. All configuration variables will also be automatically destroyed.
 */
extern void Config_section_destroy(T section);

/**
 * Request a variable from the section by the varname. NULL will be returned if the variable does
 * not exist.
 */
extern Config_value_T Config_section_get(T section, const char *varname);

/**
 * Set the specified variable and value into the specified section. Overwriting varname
 * is permitted, with the overwritten value safely handled.
 */
